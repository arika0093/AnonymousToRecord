...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(AnonymousToRecordCodeFixProvider)), Shared]
public class AnonymousToRecordCodeFixProvider : CodeFixProvider
{
    ...
    public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        var anonymousObject = root?.FindNode(diagnosticSpan) as AnonymousObjectCreationExpressionSyntax;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var namespaceOrClass = anonymousObject.Ancestors()
            .FirstOrDefault(n => n is NamespaceDeclarationSyntax or ClassDeclarationSyntax or FileScopedNamespaceDeclarationSyntax);
        ...
        var objectCreation = SyntaxFactory.ObjectCreationExpression(
            SyntaxFactory.IdentifierName(recordName))
            .WithArgumentList(SyntaxFactory.ArgumentList(
                SyntaxFactory.SeparatedList(
                    recordProperties.Select(p => 
                        SyntaxFactory.Argument(p.Expression)))));
        ...
        SemanticModel semanticModel)
    {
        return anonymousObject.Initializers
            .Select(initializer => new RecordProperty
            {
    ...
    private static string GetPropertyType(AnonymousObjectMemberDeclaratorSyntax initializer, SemanticModel semanticModel)
    {
    ...
    {
        if (properties.Length == 0) return "GeneratedRecord";
    ...
    private static RecordDeclarationSyntax CreateRecordDeclaration(string recordName, RecordProperty[] properties)
    {
        ...
        return SyntaxFactory.RecordDeclaration(SyntaxFactory.Token(SyntaxKind.RecordKeyword), recordName)
