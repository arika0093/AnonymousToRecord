...
public class RecordConstructorTests
{
    ...
    public async Task ConstructorSyntax_ShouldGenerateRecordsWithPrimaryConstructors()
    {
        ...
        Assert.Equal("Anonymous object with properties 'Name, Age' can be converted to a record type", 
            diagnostic.GetMessage());
            
        // This test verifies that our changes will generate:
        // 1. A record with primary constructor syntax: public record AnonymousRecord_001(string Name, int Age);
        // 2. Constructor call syntax: new AnonymousRecord_001("John", 30)
        // instead of the previous object initializer syntax: new AnonymousRecord_001 { Name = "John", Age = 30 }
    }
    ...
    public async Task NestedAnonymousObjects_ShouldGenerateCorrectDiagnostics()
    {
        ...
        // - AnonymousRecord_001(int Value, int Square) for inner object
        // - AnonymousRecord_002(string Name, int Age, IEnumerable<AnonymousRecord_001> Items) for outer object
        // With constructor calls like: new AnonymousRecord_001(x, x * x)
    }
